...
try{
 ...
 await page.getByRole('link',{name:'All work',exact:true}).click();check('All work filter',await page.locator('.product-card').count()===8);
 ...
 await page.goto(origin+'/business-payments/apply');await page.getByRole('button',{name:/3.*Confirm/}).click();check('Cannot skip required company fields',await page.locator('#company_name').isVisible());check('Invalid first step is focused',await page.locator('#company_name').evaluate(node=>node===document.activeElement));
